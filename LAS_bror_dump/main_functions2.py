...
class add(parentFunction):
    ...
    def __call__(self, *args):
        ...
        if isinstance(self.call_arg, Variable):
            ...
            init_args_dummy = [self.call_arg if isinstance(obj,Variable) else obj for obj in self.init_args]
            res = 0
            ...
            try:
                if isinstance(self.call_arg.call_arg, Variable) or (self.call_arg.call_arg is None):
                    # this will succeed if f or f(x) is called.
                    new_init_args = [self.call_arg if isinstance(obj, Variable) else obj for obj in self.init_args]
